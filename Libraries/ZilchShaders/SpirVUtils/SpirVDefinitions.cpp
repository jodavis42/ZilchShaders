...
void SpirVInstructions::Load(Zilch::JsonValue* jsonRoot)
{
  if(jsonRoot == nullptr)
    return;
  ...
  mIsSimplifiedInstructions = false;
  mInstructions.Clear();
  Zilch::JsonValue* instructionsGrammar = jsonRoot->GetMember("instructions", Zilch::JsonErrorMode::DefaultValue);
...
void SpirVInstructions::ParseInstructions(Zilch::JsonValue* instructionsJson, HashMap<String, Instruction>& instructions)
{
...
void SpirVInstructions::ParseInstruction(Zilch::JsonValue* instructionJson, HashMap<String, Instruction>& instructions)
{
  Zilch::JsonValue* opNameGrammar = instructionJson->GetMember("opname");
  Zilch::JsonValue* opCodeGrammar = instructionJson->GetMember("opcode");
  ...
  Zilch::JsonValue* operandsGrammar = instructionJson->GetMember("operands", Zilch::JsonErrorMode::DefaultValue);
  if(operandsGrammar != nullptr)
    ParseOperands(operandsGrammar, instruction.mOperands);
}

void SpirVInstructions::ParseOperands(Zilch::JsonValue* operandsJson, Array<Instruction::Operand>& operands)
{
  for(size_t i = 0; i < operandsJson->ArrayElements.Size(); ++i)
  {
    Zilch::JsonValue* operandGrammar = operandsJson->ArrayElements[i];
    Instruction::Operand& operand = operands.PushBack();
    
    Zilch::JsonValue* kind = operandGrammar->GetMember("kind", Zilch::JsonErrorMode::DefaultValue);
    if(kind != nullptr)
      operand.mKind = kind->AsString();
    Zilch::JsonValue* name = operandGrammar->GetMember("name", Zilch::JsonErrorMode::DefaultValue);
    if(name != nullptr)
...
void SpirVDefinitions::LoadCore(StringParam filePath)
{
  ...
  Zilch::JsonValue* valueRoot = Zilch::JsonReader::ReadIntoTreeFromFile(errors, filePath, nullptr);
