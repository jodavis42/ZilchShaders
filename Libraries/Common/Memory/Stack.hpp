...
public:
  ...
  Stack(cstr name, GraphNode* parent, size_t stackSize, size_t maxEntries);
  ~Stack();
...
public:
  ...
  Zero::Memory::MemPtr Allocate(size_t numberOfBytes){return mStack->Allocate(numberOfBytes); };
  void Deallocate(Zero::Memory::MemPtr ptr, size_t numberOfBytes){mStack->Deallocate(ptr, numberOfBytes);}
  Memory::Stack* mStack;
};
