...
void ByteBuffer::Append(const byteType* data, size_t sizeInBytes)
{
  if(mCurBlockBuffer == nullptr)
  {
    ...
    mCurBlockBuffer = (byteType*)Memory::zAllocate(mBlockSize);
    mCurBlockSize = 0;
...
void ByteBuffer::ExtractInto(ByteBufferBlock& buffer) const
{
  ...
  buffer.mData = (byte*)Memory::zAllocate(mTotalSize);
...
void ByteBuffer::Deallocate()
{
  ...
  for(;!blocks.Empty();blocks.PopFront())
  {
    Memory::zDeallocate(blocks.Front());
  }
  ...
  mOwnsData(rhs.mOwnsData)
{
  if (mOwnsData)
  {
    mData = (byte*)Memory::zAllocate(rhs.mSize);
...
{
  mData = (byte*)Memory::zAllocate(size);
  mCurrent = mData;
  mSize = size;
  mOwnsData = true;
}
