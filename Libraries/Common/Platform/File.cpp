...
byte* ReadFileIntoMemory(cstr filePath, size_t& fileSize, size_t extra)
{
  ...
  byte* fileBuffer = (byte*)Memory::zAllocate(fileSize + extra);
...
void WriteToFile(StringParam path, StringParam fileContents)
{
  WriteToFile(path.c_str(), (byte*)fileContents.Data(), fileContents.SizeInBytes());
}

void WriteToFile(StringParam path, StringRange fileContents)
{
  WriteToFile(path.c_str(), (byte*)fileContents.Data(), fileContents.SizeInBytes());
}
