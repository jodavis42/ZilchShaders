...
bool TestDiff2(StringParam expectedFilePath, StringParam generatedFilePath, StringParam generatedFileContents, ErrorReporter& reporter)
{
  ...
  {
    WriteToFile(expectedFilePath, String(""));
  }
...
void TestCompilation(SimpleZilchShaderIRGenerator& shaderGenerator, ZilchShaderIRBackend* backendTranslator, ZilchShaderSpirVSettings* settings, ZilchShaderIRType* type, StringParam directory, ErrorReporter& reporter)
{
  ...
  WriteToFile(generatedFilePath, generatedResults.All());
...
void ReportErrors(SimpleZilchShaderIRGenerator& shaderGenerator, StringParam directory, ErrorReporter& reporter)
{
  ...
  for(size_t i = 0; i < shaderGenerator.mFragmentProject.mCodeEntries.Size(); ++i)
  {
    ...
    WriteToFile(generatedFilePath, errorStr);
...
void TestCompositesOfDirectory(SimpleZilchShaderIRGenerator& shaderGenerator, UnitTestPackage& unitTestPackage, Array<ShaderDefinition>& shaders, StringParam directory, ErrorReporter& reporter)
{
  LoadDirectorySettingsOrDefault(*shaderGenerator.mSettings, directory);
  ...
  {
    ShaderDefinition& shaderDef = shaders[i];
    TestCompositeDefinition(shaderGenerator, shaderDef, unitTestPackage, directory, reporter);
  }
}

void TestCompositeDefinition(SimpleZilchShaderIRGenerator& shaderGenerator, ShaderDefinition& shaderDef, UnitTestPackage& unitTestPackage, StringParam directory, ErrorReporter& reporter)
{
  ...
  for(size_t i = 0; i < FragmentType::Size; ++i)
  {
    ...
    WriteToFile(generatedFilePath, shaderInfo.mShaderCode);
...
void TestDirectory(SimpleZilchShaderIRGenerator& shaderGenerator, UnitTestPackage& unitTestPackage, StringParam directory, ErrorReporter& reporter, bool recurse)
{
  ...
  Array<ShaderDefinition> shaders;
  LoadZilchScriptShaderDefinitions(testConfig, shaders, directory);

  if(testConfig.mTestComposites)
    TestCompositesOfDirectory(shaderGenerator, unitTestPackage, shaders, directory, reporter);
  else
...
void TestShaderDefinition(SimpleZilchShaderIRGenerator& shaderGenerator, RendererPackage& package, ShaderDefinition& shaderDef)
{
